...
```{bash, eval=FALSE}
samtools sort -n original.bam -o sorted_by_name.bam #sort reads by name
samtools view -b -F 256 sorted_by_name.bam -o primary_alignment_only.bam # remove secondary alignments
bedtools2/bin/badtools bamtofastq -i primary_alignment_only.bam -fq read1.fq -fq2 read2.fq # convert to fastq
```
## CRAM
...
samtools view -h file.[cram/bam] | wc -l # counts the number of lines in the samtools output
```
...
samtools view -H EXAMPLE.cram | more
samtools view -f 4 EXAMPLE.cram | wc -l # unmapped
samtools view -F 260 EXAMPLE.cram | wc -l # mapped
samtools view -F 256 EXAMPLE.cram | wc -l # total
samtools view -f 256 EXAMPLE.cram | wc -l # secondary alignments
...
bedtools bamtofastq -i primary_EXAMPLE.bam -fq 10cells_read1.fq -fq2 10cells_read2.fq # convert to fastq
...
Another is to fix a mixture model and find where the higher and lower distributions intersect.
...
require("mixtools")
mix <- normalmixEM(umi_per_barcode)
plot(mix)
p1 <- dnorm(umi_per_barcode, mean=mix$mu[1], sd=mix$sigma[1])
p2 <- dnorm(umi_per_barcode, mean=mix$mu[2], sd=mix$sigma[2])
...
```
Finally EmptyDrops, which is currently in beta testing, uses the full genes x 
